...
export default function CekDomainClient({ fields }: { fields: Record<string, string> }) {
  ...
  const handleCheck = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setResults(POPULAR_TLDS.map((tld) => ({ domain: base + tld, available: Math.random() > 0.4 })));
      setLoading(false);
    }, 1200);
  };
    ...
    <main className="pt-16">
      ...
      <section style={{ backgroundColor: "#200033" }} className="py-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="inline-block mb-4 text-xs font-semibold tracking-widest uppercase" style={{ color: "rgba(255,255,255,0.6)" }}>{f("hero_label")}</span>
          <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-3 leading-tight">{f("hero_heading")}</h1>
          <h2 className="text-2xl sm:text-3xl font-bold mb-4" style={{ color: "rgba(255,255,255,0.85)" }}>{f("hero_subheading")}</h2>
          <p className="text-base mb-10" style={{ color: "rgba(255,255,255,0.65)" }}>{f("hero_text")}</p>
          <form onSubmit={handleCheck} className="flex flex-col sm:flex-row gap-3 max-w-2xl mx-auto">
            ...
            />
            <button type="submit" disabled={loading} className="px-8 py-4 rounded-xl font-bold text-white text-base transition-opacity hover:opacity-90 disabled:opacity-60 shrink-0" style={{ backgroundColor: "#480E6A" }}>
              {loading ? f("button_checking") : f("button_check")}
            </button>
          </form>
        </div>
      </section>
      ...
      {results && (
        <section className="py-16 bg-white">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-extrabold mb-8 text-center" style={{ color: "#200033" }}>{f("results_heading")}</h2>
            <div className="space-y-3">
              {results.map((r) => (
                <div key={r.domain} className="flex items-center justify-between px-6 py-4 rounded-xl border" style={{ borderColor: r.available ? "#d1fae5" : "#fee2e2", backgroundColor: r.available ? "#f0fdf4" : "#fff5f5" }}>
                  <span className="font-semibold text-base" style={{ color: "#200033" }}>{r.domain}</span>
                  <div className="flex items-center gap-3">
                    <span className={`text-sm font-semibold px-3 py-1 rounded-full ${r.available ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>{r.available ? f("available") : f("not_available")}</span>
                    {r.available && (
                      <a href={WA} target="_blank" rel="noopener noreferrer" className="text-xs font-bold px-4 py-2 rounded-lg text-white hover:opacity-90 transition-opacity" style={{ backgroundColor: "#480E6A" }}>{f("register")}</a>
                    )}
                  </div>
                </div>
              ))}
            </div>
            <p className="text-xs text-center mt-6" style={{ color: "#554B4E" }}>{f("whatsapp_note")}</p>
          </div>
        </section>
      )}
      ...
      <section className="py-20" style={{ backgroundColor: "#f9f7ff" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <span className="inline-block mb-3 px-4 py-1 rounded-full text-xs font-semibold text-white" style={{ backgroundColor: "#480E6A" }}>{f("why_label")}</span>
            <h2 className="text-3xl sm:text-4xl font-extrabold" style={{ color: "#200033" }}>{f("why_heading")}</h2>
            <p className="mt-3 text-base max-w-2xl mx-auto" style={{ color: "#554B4E" }}>{f("why_text")}</p>
          </div>
